  ...
  "review-workflow": {
    ...
    async run(page) {
      ...
      const headerLayout = await page.locator("[data-app-header]").evaluate((header) => ({
        ...
        // Rendered children only: the Build/Review switch is in the header but
        // hidden outside a studio, and a display:none box has no centre to align.
        centers: [...header.children].filter((child) => child.getClientRects().length > 0).map((child) => {
