...
class TestModelPatch(unittest.TestCase):
    ...
    def test_patch_rolls_back_group_mutation_on_failure(self):
        model = Model(project_name="GroupRollback")
        model.add_material("Steel", E=2.0e11, nu=0.3)
        model.add_pipe_section("PipeSec", OD=0.1, WT=0.01)
        model.groups["existing_group"] = {"name": "existing_group", "nodes": [], "elements": [], "supports": [], "metadata": {}}
        patch = ModelPatch(
            operations=[
                CreateGroup(name="new_group", nodes=[], elements=[]),
                AddElement(
                    local_id="bad",
                    type="pipe_straight",
                    n1="missing_1",
        ...
        self.assertNotIn("new_group", model.groups)
        self.assertIn("existing_group", model.groups)

    def test_find_node_by_point_euclidean_distance_at_large_coordinates(self):
        model = Model(project_name="EuclideanSnapping")
        n0 = model.add_node((1000.005, 0.0, 0.0))
        # With tol=1e-6 (1 micron), 5mm delta must NOT match (no rtol coordinate scaling artifact)
        match = model.find_node_by_point((1000.0, 0.0, 0.0), tol=1e-6)
        self.assertIsNone(match)
        # With tol=0.01 (10mm), 5mm delta MUST match
        match_coarse = model.find_node_by_point((1000.0, 0.0, 0.0), tol=0.01)
        self.assertEqual(match_coarse, n0)
