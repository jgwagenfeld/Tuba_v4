...
def test_gmsh_mesh_viewer_recipe_builds_an_unsolved_scene() -> None:
    ...
    tee = load_project(Path(__file__).resolve().parents[1] / "examples" / "pipe-tee-volume-review")
    scene = tee.load_study("mesh_study.py").build()
...
def test_beam_comparison_validates_each_load_case_evidence(tmp_path: Path) -> None:
    ...
    root = study.build_review(project.run_model(), tmp_path, artifact_dir=study.ARTIFACT_DIR)
...
def test_gallery_evidence_lives_in_its_project_one_folder_per_operation() -> None:
    ...
    listed = subprocess.check_output(["git", "ls-files", "-z", "--", ":(glob)examples/*/evidence/**"], cwd=REPO_ROOT)
