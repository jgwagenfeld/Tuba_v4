...
def test_refresh_uses_native_volume_export_for_the_tee_gallery(tmp_path, monkeypatch):
    ...
    # The volume options come from the tee project's study, through the gallery record.
    volume = refresh_code_aster_gallery._gallery_record("pipe-tee-volume-review").volume_export
    assert volume == {"element_ids": ("header_left", "header_right", "branch"), "max_element_size": 0.005}
    assert calls[0][4]["element_ids"] == volume["element_ids"]
    assert calls[0][4]["max_element_size"] == volume["max_element_size"]
...
def test_artifact_review_uses_the_observed_solve_timestamp(tmp_path):
    ...
    def build_model():
        return load_project(Path(__file__).resolve().parents[1] / "examples" / "code-aster-review").run_model()["model"]
...
def test_bend_export_preserves_windows_executable_search(tmp_path):
    ...
    CodeAsterSolver(work_dir=tmp_path).export_analysis_study(
        load_project(Path(__file__).resolve().parents[1] / "examples" / "code-aster-review").run_model()["model"],
        "Operating",
        tmp_path,
    )
