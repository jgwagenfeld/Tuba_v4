...
DEFAULT_SNAPPING_TOLERANCE: float = 1e-6
...
class TubaModel:
    ...
    def find_node_by_point(
        self, coords, *, tol: float = DEFAULT_SNAPPING_TOLERANCE
    ) -> Optional[str]:
        """Return an existing node id at coords, if one exists within tolerance."""
        target = np.asarray(coords, dtype=float)
        tol_val = float(tol)
        indexed = self._node_point_index.get(_point_index_key(target, tol_val))
        if indexed is not None:
            node = self.nodes.get(indexed)
            if node is not None and float(np.linalg.norm(node.coords - target)) <= tol_val:
                return indexed
